...
class AsymmetricPIDControls(QWidget):
    ...
    def setup_ui(self):
        ...
        # Asymmetric Autotune
        autotune_group = QGroupBox("🎯 Asymmetrisk autotune")
        ...
        autotune_layout.addWidget(guidance_group)
        ...
        autotune_layout.addWidget(self.autotune_results_panel)
...
class MainWindow(QMainWindow):
    ...
    def handle_autotune_results(self, results: Dict[str, Any]):
        ...
        try:
            ...
            if hasattr(self, "asymmetric_controls"):
                self.asymmetric_controls.show_autotune_results(results)
            else:
                QMessageBox.information(
                    self,
                    "Autotune-resultater",
                    "Autotune er ferdig. Kontroller loggen for detaljer.",
                )
            ...
            if status == "failed":
                self.log("⛔ Autotune failed – insufficient response", "error")
            elif status == "partial":
                self.log(
                    "⚠️ Autotune delivered partial data → "
                    + ("; ".join(log_parts) if log_parts else "ingen gyldige verdier"),
                    "warning",
                )
            else:
                self.log(
