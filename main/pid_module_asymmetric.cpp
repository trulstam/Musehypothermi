...
void AsymmetricPIDModule::update(double /*currentTemp*/) {
    ...
    finalOutput = smoothedCorrection;

    double correctionOutput = finalOutput;
...
void AsymmetricPIDModule::applyRateLimiting() {
    ...
    double delta = rawPIDOutput - lastCommandedOutput;
    if (delta > maxDelta) {
        rawPIDOutput = lastCommandedOutput + maxDelta;
